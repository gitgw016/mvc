...
UPDATE mvc_member SET id = 'baesin@betray.bsj' where num = 4;
...
INSERT INTO notice_board VALUES(null, '공지', '관리자','뭐','ㅋㅋ',now());
...
CREATE TABLE qna_board(
	...
	qna_re_seq INT NOT NULL,								-- 단변글 정렬 번호
	qna_writer_num INT,										-- 글 작성자 번호
	qna_readcount INT DEFAULT 0,							-- 조회수n 
	qna_date TIMESTAMP DEFAULT now(),						-- 작성시간
	FOREIGN KEY(qna_writer_num) REFERENCES mvc_member(num)
);

ALTER TABLE qna_board ADD qna_delete char(1) DEFAULT 'N' AFTER qna_readcount;	-- 삭제 여부(추가)
